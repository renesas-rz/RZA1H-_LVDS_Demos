...
rem * Above search code gratefully provided by TES Guiliani *
    ...
    set SelectedFile=UserData.bin 
    copy /y .\%toolsdir%\program_qspi_la_head.Command .\LoadUserD.Command
    echo loadbin %toolsdir%\%SelectedFile%,0x19000000>> LoadUserD.Command
    copy /y .\LoadUserD.Command + %toolsdir%\program_qspi_la_tail.Command LoadUserData.Command    
   "%programmer%%programmer2%" -speed 12000 -if JTAG -JTAGConf -1,-1 -device R7S721001_DualSPI -CommanderScript LoadUserData.Command
